...
public class R2Service extends FileManagerService {

    @Value("${CLOUDFLARE_R2_BUCKET:default}")
    private String bucketName;

    @Value("${CLOUDFLARE_R2_ACCOUNT_ID:default}")
    private String accountId;

    @Value("${CLOUDFLARE_R2_ACCESS_KEY:default}")
    private String accessKey;

    @Value("${CLOUDFLARE_R2_SECRET_KEY:default}")
    private String secretKey;

    @Value("${CLOUDFLARE_R2_SESSION_TOKEN:default}")
    ...
    public void init() {
        ...
        if (StringUtils.hasText(endpoint)) {
            ...
            endpointUrl = String.format("https://%s.r2.cloudflarestorage.com", accountId);
        }

        S3ClientBuilder builder = S3Client.builder()
                .endpointOverride(URI.create(endpointUrl))
                .region(Region.of("auto")); // R2 doesn't use regions like AWS, 'auto' is commonly used
        ...
        this.storageType = StorageType.CLOUDFLARE_R2;
    }
    ...
    public Page<File> list(Pageable pageable, String path) {
        try {
            ...
            throw new RuntimeException("Cloudflare R2 credentials not configured or invalid.", e);
        }
    }
